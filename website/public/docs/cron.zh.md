...
在 NousAIPaw 里，「定时任务（cron job）」用于让系统在指定时间自动执行动作，比如：
...
创建定时任务最简单的方式是直接与 NousAIPaw 对话，让NousAIPaw帮忙创建：
...
NousAIPaw 使用五段式 Cron：**分 时 日 月 周**（无秒）。
