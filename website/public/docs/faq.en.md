...
### NousAIPaw vs OpenClaw: Feature Comparison

Please check the [Comparison](/docs/comparison) page for detailed feature comparison.

### How to install NousAIPaw

NousAIPaw supports multiple installation methods. See
...
If you previously used CoPaw, upgrading to NousAIPaw only requires downloading the latest NousAIPaw. No extra migration is needed; your configuration, memory, skills, and other data from the CoPaw era continue to work.

### How to initialize and start NousAIPaw service
...
with NousAIPaw's default port **8088**. This affects all installation methods
...
- NousAIPaw fails to start, or browser cannot connect to `http://127.0.0.1:8088/`
...
### APITimeoutError when running NousAIPaw in WSL2 (NAT mode)

When running NousAIPaw inside WSL2 with NAT networking (especially when a VPN is
...
After this, NousAIPaw should be able to communicate with model providers
normally.

### Open-source repository

NousAIPaw is open source. Official repository:
...
or NousAIPaw GitHub [Releases](https://github.com/agentscope-ai/QwenPaw/releases).
...
QwenPaw-Flash is a family of models tuned by the NousAIPaw team for NousAIPaw's core
...
All built-in local providers in NousAIPaw can be used with QwenPaw-Flash models:

#### NousAIPaw Local (llama.cpp)

In the NousAIPaw Local model interface, simply choose a QwenPaw-Flash model to
...
> NousAIPaw Local is still in beta. Compatibility across different devices and
> runtime stability are still being improved. If you run into issues while
> using it, please open an issue on GitHub.
> If NousAIPaw Local does not work properly in your environment, we recommend
...
4. In NousAIPaw model settings, choose the Ollama provider, then automatically load
...
4. In NousAIPaw model settings, choose the LM Studio provider, then automatically
   load the model on the Models page.

### When using models deployed with Ollama / LM Studio, why can't NousAIPaw complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a NousAIPaw bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, NousAIPaw may show problems such as:
...
- Before running NousAIPaw, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

> ⚠️ **Before running NousAIPaw, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, NousAIPaw typically needs a
...
The easiest way to create a cron job is to talk to NousAIPaw in the channel where you want the results. For example, say: “Create a scheduled task that reminds me to drink water every five minutes.” You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the NousAIPaw service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask NousAIPaw: “Trigger the ‘drink water reminder’ task I just created.”
...
NousAIPaw supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/QwenPaw/issues) in the NousAIPaw GitHub
...
and also provide your current model provider, model name, and NousAIPaw version.
