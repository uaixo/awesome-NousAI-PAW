...
NousAIPaw 支持**多智能体**，允许您在同一个 NousAIPaw 实例中运行多个独立的 AI 智能体。
...
简单来说，**多智能体**就是让您可以在一个 NousAIPaw 中运行多个"分身"，每个分身：
...
启动 NousAIPaw 后，在控制台**左上角**可以看到**智能体切换器**：
...
如果您之前使用 NousAIPaw **v0.0.x**，升级到 **v0.1.0** 时会**自动迁移**：
...
除了与**不同 workspace 的其他 Agent** 协作（`chat_with_agent`），NousAIPaw 还支持在**当前项目内**派发临时子任务。
...
NousAIPaw 在创建 worktree 时会自动将这些文件复制过去，确保子任务能正常运行。
