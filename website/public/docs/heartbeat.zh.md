...
「心跳」在 NousAIPaw 里指的是：**按固定间隔，用你写好的一段「问题」去问 NousAIPaw，并可选择把 NousAIPaw 的回复发到你上次对话的频道**。适合做「定期自检、每日摘要、定时提醒」——不用你主动发消息，NousAIPaw 到点就干活。
...
1. 在当前智能体的 workspace 里有一个**心跳查询文件**（默认文件名为 **HEARTBEAT.md**，可用环境变量 `QWENPAW_HEARTBEAT_FILE` 改名）。里面写的是**每次心跳要问 NousAIPaw 的内容**（一段或几段话都行，NousAIPaw 会当成一条用户消息）。
2. 当配置里 **`enabled` 为 true** 时，系统按你配置的 **every**（间隔字符串或五段 Cron）执行一次：读取该文件 → 用这段内容去问 NousAIPaw → NousAIPaw 回复。
3. **发不发到频道** 由配置里的 **target** 决定：
   - **main**：只跑 NousAIPaw，不把回复发到任何频道（适合只做「自检」、结果自己看日志或别处）。
   - **last**：把 NousAIPaw 的回复发到你**上次和 NousAIPaw 对话的那个频道/会话**（例如上次你在钉钉和它聊，这次心跳的回复就发到钉钉）。
...
内容就是「每次要问 NousAIPaw 什么」，纯文本或 Markdown 都行，NousAIPaw 会整体当作一条用户消息。
...
示例（开启心跳，只跑 NousAIPaw、不发到频道，每 30 分钟）——写在对应智能体的 **`agent.json`** 中：
